...
#'
#' @return A data.frame containing doublet scores and predictions for each barcode.
#'
#' @export
#'
...
MarkDoublets <- function(seu, PN = NULL){
  ...
  counts_matrix <- seu@assays$RNA@counts %>% t()
  ...
  py$scipy <- import("scipy.sparse", convert = FALSE)
  prepare_counts_matrix <- function(seu_counts_t) {
    if (inherits(seu_counts_t, "dgCMatrix")) {
      return(py$scipy$csc_matrix(seu_counts_t))
    } else {
